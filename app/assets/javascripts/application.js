...
function displayIconMenuTeam (){
	...
	var regexp = /teams/gi;
	if(url.match(regexp)){
		$('span.teams.icon-gly.glyphicon.glyphicon-triangle-top.icon-triagle-top').css({
			'display' : 'block'
		})
	}
}
...
//init datetimepicker
  $(function() {
    ...
    $('#datepicker-workload').datetimepicker(
     {
       format: 'YYYY-MM-DD'
     }).on('dp.change', function (e) {
       date = $(this).val()
  ...
  $('#createWorkflow').on('click', function(){
    idWorkflow = 1;
    var html_workflow = '<div class="new-workflow-step">'+
    '<div class="col-md-2 step-lable">'+idWorkflow+'</div>'+
      '<div class="col-md-10 input-step">'+
        '<input class="speed-input input-step-workflow" name="step'+ idWorkflow +'[name]" placeholder="Step ">'+
        '<p class="set-dealine input-group form-group speed-date-input">'+
          '<span class="input-group-addon"><span class="glyphicon glyphicon-time set-dealine"></span>'+
          //'</span>'+
          '<input name="step'+ idWorkflow +'[date]" placeholder="Click here to add estimate time" type="text">'+
        '</p>'+
      '</div>'+
      '<div class="icon-delete" id="delete-workflow-step">'+
  ...
  // var html_form_edit_comment = "<div class='row'><div class='col-md-12'><div class='input text required comment_content'><label class='text required' for='comment_content'></label><textarea class='comment-editing text required form-control' name='comment[content]' id='comment_content'></textarea></div></div> <button id='save-comment-btn' class='btn speed-btn speed-comment-btn'> save </button></div>"
  //
  // // var comment_content = $('.comment-content .comment-user-content');
  // var $selector_comment = $('.comment-content-wrapper .comment-content');
  // var $edit_btn = $selector_comment.find('span.edit');
  //
  ...
  //   var comment_content_selector = $(this).parent().parent().parent().find('.comment-user-content');
  //   var comment_control_selector = $(this).parent().parent().parent().find('.comment-control');
  //   var comment_content = comment_content_selector.text();
  //   //hide current comment;
  //   $(comment_content_selector).hide();
  //   $(comment_control_selector).hide();
  //
  //   $(comment_content_selector).parent().find('.form-hidden-comment').append(html_form_edit_comment);
  //   //add current content;
  //   $(comment_content_selector).parent().find('textarea.comment-editing').val(comment_content);
  //
  //   $('#save-comment-btn').on('click', function() {
  //     $('.comment-content-user').remove();
  //     value = $(this).parent()
  //     id = value.parent().parent().parent().find('.comment-control').find('input').val()
  //     $.ajax({
  //       type: "PUT",
  //       url: "/comments/"+id,
  //       dataType: "json",
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      $.each(result[0]["step"], function(k,v) {
        ...
        string = '<div class="row workflow-step">'+
        '<div class="col-md-12 step-name">'+v.name+'</div>'+
        '<div class="row">'+
        '<div class="col-md-5">'+
          '<input id="datetimepicker'+id+'" class="speed-input remove-margin remove-padding date-week require" name="deadline-'+v.id+'" placeholder="Add deadline" type="text">'+
        '</div>'+
          '<div class="col-md-7 step-assign-to">'+
            '<span class="col-md-4 title remove-margin remove-padding"> Assign to: </span>'+
            '<select class="col-md-8 speed-input remove-margin remove-padding select2 member-team" name="name_step_'+v.id+'"></select2>'+
          '</div>'+
        '</div>'+
        '</div>'
...
$(document).on('ready', function(){
  ...
  $('.checkbox.check_list').find('input').on('click',function(){
    ...
    $('.checkbox.check_list').find('input').each(function() {
      if (string=="{")
        string = string + '"' + $(this).attr('name') + '":"' + ($(this).is(":checked")) + '"';
      else
          string = string + ',"' + $(this).attr('name') + '":"' + ($(this).is(":checked")) + '"';
    });
    string = string + '}';
    ...
    $("#more-activities").on('click', function() {
      ...
      $.ajax({
        ...
        $.each(JSON.parse(result), function(k,v) {
            string = '<tr class="user-info">'+
            '<td class="col-xs-3 col-md-3">'+
                '<span class="user-name">'+ v.fullname+'</span>'+
              '</td>'+
            '<td class="col-xs-6 col-md-6">'+
              '<span class="user-title">'+ v.event + '&nbsp' + v.category +
                '<strong> '+ v.name +'</strong>'+
              '</span>'+
            '</td>'+
            '<td class="col-xs-3 col-md-3"> &nbsp'+
              '<span> '+
                v.datetime +
              '</span>'+
            '</td>'+
...
$(document).ready(function(){
  ...
  $('.today-task-items').each(function(){
    var set_with_todo = $(this).find('.task-item').width()
                    - $(this).find('.user-name-noti').outerWidth()
                    - $(this).find('.event-noti').outerWidth()
                    - $(this).find('.timestamp-noti').outerWidth() -35;
          ...
          $.each(JSON.parse(result), function(k,v) {
            if ($.inArray($('#id_user').val(),v.user_list)==-1)
              if (v.category == "todo list")
                string = '<div class="wrapper-one-todo-list" style="background-color: #F5F5F5">'+
                          '<div class="todo-list-name">'+ v.parent_id + '</div>' +
                          '<div class="row today-task-items"><div class="col-md-12 task-item">' +
                          '<div class="user-name-noti">' +  v.user +
                          '</div><div class="event-noti">&nbsp' + v.event + ' on </div>' +
                          '<div class="catagory-noti limit-text" >' +
                          '<a data-backdrop="static" data-toggle="modal" href="'+ v.url +'">&nbsp' + v.name + '</a></div>' +
                          '<div class="timestamp-noti"> at ' + v.date +
                          '</div> </div> </div> </div>';
              else
                string = '<div class="wrapper-one-todo-list" name="'+v.id+'"  style="background-color: #F5F5F5">'+
                          '<div class="todo-list-name">'+ v.parent_id + '</div>' +
                          '<div class="row today-task-items"><div class="col-md-12 task-item">' +
                          '<div class="user-name-noti">' +  v.user +
                          '</div><div class="event-noti">&nbsp' + v.event + ' on </div>' +
                          '<div class="catagory-noti limit-text" >' +
                          '<a data-backdrop="static" data-target="#foxyModal" data-toggle="modal" href="'+ v.url +'">&nbsp' + v.name +      '</a></div>' +
                          '<div class="timestamp-noti"> at ' + v.date +
                          '</div> </div> </div> </div>';

            else
              if (v.category == "todo list")
                string = '<div class="wrapper-one-todo-list" style="background-color: fff">'+
                          '<div class="todo-list-name">'+ v.parent_id + '</div>' +
                          '<div class="row today-task-items"><div class="col-md-12 task-item">' +
                          '<div class="user-name-noti">' +  v.user +
                          '</div><div class="event-noti">&nbsp' + v.event + ' on </div>' +
                ...
                string = '<div class="wrapper-one-todo-list" name="'+v.id+'" style="background-color: fff">'+
                          '<div class="todo-list-name">'+ v.parent_id + '</div>' +
                          '<div class="row today-task-items"><div class="col-md-12 task-item">' +
                          '<div class="user-name-noti">' +  v.user +
                          '</div><div class="event-noti">&nbsp' + v.event + ' on </div>' +
